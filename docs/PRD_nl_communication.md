...
- **Output:** updated belief (distribution or ranked hypotheses over opponent cells) — **never a crash**.
...
## 3. Belief Model
- Maintain a belief over opponent positions consistent with observations and message history.
- Update rule (conceptual): `belief' = normalize(prior ⊙ obs_likelihood ⊙ message_likelihood)`.
- The Thief may emit deceptive messages → the decoder weights message evidence below hard observation.
...
- **Mutual understanding:** include a brief, role-appropriate prompt contract so both sides converge.
