...
| A15 | 2026-06-26 | Partner can't drive our FastMCP; explain it + onboard them | "Is it our or their architecture mistake? Build our `/decide`; write a detailed prompt telling their Codex the problem, what the assignment wants, and how to use our MCP **and** our REST." | `docs/PARTNER_ONBOARDING.md` (problem → target architecture → REST path + MCP path with tool schemas → report); README R.12 'whose architecture' + MCP-transport caveat; ADR-009 | Be precise about fault vs. who-adapts: the spec puts the brain in the *client* (tools-only server) — we comply, they deviated — but "their fault" ≠ "they must fix it"; the cheapest bridge (our `/decide`) won, and we documented the spec-correct path too so the choice is theirs. |
| A16 | 2026-06-26 | Final inter-group submission (config-driven `bonus_game` + loser=7) | "Add the lecturer to recipients, run the full game, build the bonus_game report, email all, commit + PR + merge." | `reporting.build_interop_bonus_report` (config-driven §9.2) + `config.intergroup`; `config.bonus.lose=7` (lecturer); `play_partner.py` emails JSON-only to all recipients; live **60-40** emailed to lecturer + both teams | The lecturer resolved the assignment's 5-vs-7 contradiction (loser=7); kept the report config-driven (no hardcoded opponent data) so it's reproducible; flagged that §12.2 needs salareen to send the **identical** JSON or it's 0/0. |
| _…_ | | | | | |
